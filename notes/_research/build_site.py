...
ORDERS = [
 ...
 dict(kind="机票", what="SK4787 奥斯陆 → 冰岛（SAS · 当天唯一直飞）", when="9/25 计划 06:15 → 07:05 · 实际 06:32 起飞",
      money="已出票", sub="", cxl="—"),
 ...
 dict(kind="活动", what="Troll.is 冰川徒步 + Jökulsárlón 皮划艇组合（4 人）", when="9/27 09:30 徒步（09:10 报到）· 13:30 皮划艇（13:10 报到）",
      money="$910", sub="已全额付款（含国家公园费）", cxl="—"),
]
...
OVERRIDE_DAYS = {
 "9/24": dict(body="傍晚的飞机落在 Gardermoen。不打车、不等班车 —— 顺着连廊走五分钟就是酒店。"
              ...
              shots=[]),
 "9/25": dict(body="SK4787（当地时间）：计划 06:15 从 OSL 起飞、07:05 抵达 KEF（飞行 2 小时 50 分）；航班动态截图显示实际 06:32 起飞，较计划晚 17 分钟。截图里更新后的预计抵达分钟被裁切，07:05 仍只表示计划到达；实际落地时间以机场动态为准。Avis 08:00 取车仍按计划落地安排，需根据实际落地和摆渡时间与柜台衔接。之后先去二十分钟外的蓝湖，把长途飞行泡掉；"
                   "中午沿雷克雅内斯半岛绕一圈：两块大陆之间的一座小桥、冒着白汽的泥浆池、被浪凿出来的石头浴缸。"
                   "傍晚才进城，那座混凝土教堂被造成玄武岩柱的样子，老港边上停着一艘钢骨头做的维京船。"),
 "9/27": dict(t="冰川徒步与冰河湖皮划艇", place="Jökulsárlón · Diamond Beach · Birkifell（夜间另看 Stokksnes）", drive="Hörgsland→集合点单程约112 km / 1h47（OSRM；路况另计）",
              body="今天的主线已锁定：09:30 在 Jökulsárlón 先参加 Breiðamerkurjökull 冰川徒步（票面 09:10 报到），13:30 同地皮划艇（票面 13:10 报到），4 人套餐已全额付清。Hörgsland 周边到集合点约112 km / 1h47（OSRM估算）；考虑票面通用说明要求至少提前30分钟，建议06:45出发、09:00到达。两项活动的结束时间票据未列，先别加固定景点或午餐；徒步/皮划艇结束后若有时间和体力，再看紧邻的钻石沙滩，然后去 Höfn 西侧的 Birkifell 入住。晚上的 Stokksnes 极光安排仍在周日例会结束22:00后，视云况决定。"),
 "9/28": dict(body="黄金圈：一道裂开的大陆缝、一口每隔几分钟就炸上来的间歇泉、一条把整条河横着切断的瀑布。"
                    "蓝湖挪到第一天之后，这天从六个半小时降到五小时。晚上住 KEF 旁边那栋带热浴桶和桑拿的房子。"),
 "9/29": dict(body="DY1171（当地时间）：9/29 20:05 从 KEF 起飞，飞行 2 小时 40 分；9/30 00:45 抵达 OSL（跨日、4 人已出票）。按计划 18:00 先还冰岛租车；白天睡到自然醒、轻松在雷市活动，并留出回机场时间。",
              stayname="Radisson Blu Airport Hotel ×2 间",
              staymeta="酒店 · 连廊直通航站楼 · 凌晨一点进房、七点五十再走 · 可退到当天 18:00",
              shots=[]),
 "9/30": dict(body="凌晨 00:45（奥斯陆当地时间）由 DY1171 抵达 OSL（9/29 20:05 从 KEF 起飞，飞行 2 小时 40 分）。按计划步行约 5 分钟走连廊，01:00 入住 Radisson，睡到 11:00、11:30 离开。接续 OSL→EVE 目前按 13:20–15:00 候选时段规划，仍待最终票据确认、并非已出票；确认后再按实际航班调整取车与驾程。"),
