...
out = pathlib.Path("out_isdom")
out.mkdir(parents=True, exist_ok=True)

JOBS = [
    # A) true zero-backtrack: fly east 9/26, then drive WEST back to KEF
    ("I-rkv-egs-0926", "RKV", "EGS", "2026-09-26"),
    ("I-rkv-egs-0925", "RKV", "EGS", "2026-09-25"),
    # B) drive east as planned, then fly back instead of re-driving Route 1
    ("I-hfn-rkv-0928", "HFN", "RKV", "2026-09-28"),
    ("I-egs-rkv-0928", "EGS", "RKV", "2026-09-28"),
    ("I-aey-rkv-0928", "AEY", "RKV", "2026-09-28"),
]


def url_for(fr, to, d):
    q = f"one-way flights from {fr} to {to} on {d} for 4 adults"
    return ("https://www.google.com/travel/flights?hl=en&curr=EUR&q="
...
with sync_playwright() as p:
    ...
    for slug, fr, to, d in JOBS:
        ...
        try:
            ...
            rows = pg.locator("li").filter(has_text=re.compile(r"€|EUR|ISK|kr"))
            offers = []
