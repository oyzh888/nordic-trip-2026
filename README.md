...
**四段租车已于 2026-09-06 全部下单** · **九晚住宿已全部下单**（Steve 2026-09-23 确认）·
9/24 洲际 · 9/25 SK4787 · 9/29 DY1171 **三段机票已出票**。

> ## 🤝 给接手的 agent：当前状态和唯一还开着的三件事（2026-09-23 23:xx）
>
> **已经完成、不要重做**：四台车（已付 $2,662.47）· 九晚住宿 · 三段北欧机票 · 后半段路线定稿（法国→葡萄牙→英国）。
>
> | 优先 | 还没做的 | 判据 |
> |---|---|---|
> | 🔴🔴 | **WF816 机票 ×4**（10/2 Leknes→特罗姆瑟），买 **¥2,138「免费退改」**那档 | 整张单子上唯一**会卖光**的东西（只剩 6 张，要 4 张）。⚠️ 不要买 ¥1,274 那档（连手提都没有） |
> | 🔴 | **蓝湖 Premium 票 ×4**，9/25 上午 **09:00** 那批 | 预约时段制，不是随到随进；9/25 09:00 就是它 |
> | 🔴🔴 | **10/6 洲际几点起飞**（要问 Kevin） | 它决定车③ 要不要把还车从 13:00 改早到 09:00（同一计费日内改早不涨价，免费取消到 10/3） |
>
> 另外两件小的：取车时在 Avis 柜台确认冰岛那台含不含**零自付** · **9/30 OSL→EVE 那班几点**（08:55 会让 9/29 夜只剩 6.8 小时，同价的 13:20 能到 10 小时以上）。
>
> 🔴 **两条硬规矩**：
> 1. **状态字段分三档，别混**：`viz/timeline.js` 里 `st:'booked'` = 真付过钱 · `st:'ok'` = 只是推荐方案 · `st:'tbd'` = 还没定。
>    **做完一件事必须回写这个字段** —— 9/23 就因为没回写，我把已订完的住宿当成「一个都没订」汇报了一次。
> 2. **确认号 / 预订号（PNR）/ 同行人姓名 / 付款信息一律不进这个仓库**（它是 public）。
>    PNR + 姓氏在很多航司站点足够查改别人的订单。下单结果回报写进 `notes/orders/`（已被 gitignore 挡住）。
>
> 改完数据后：`python3 notes/_research/build_site.py` → `wrangler deploy` → 官网生效。
> 只改 `viz/timeline.js` 和 `viz/data.js`，**别手改 `site/data.js`**（它是生成物）。
...
>   路线已于 2026-09-23 确认，含四段机票 + 三处住宿 + 逐日「几点要腾出来开会」
>
> ### BUY.md 原说明
...
### 配色：一条「色相旅程」，不是一堆强调色
`site/style.css` 顶部有完整理由。要点：**颜色按位置分配，不按状态分配。**
整页只有一个当前强调色 `--acc`，它随段落沿五个色停走一遍
（奥斯陆冷钢蓝 → 冰岛冰川青 → 罗弗敦海绿 → 特罗姆瑟极光绿 → 回程暮金），
所以**一屏之内只会看到一种色相**，换色发生在段落之间。
- 五个色停的明度/彩度刻意做得接近 → 相邻两段之间是过渡，不是跳色。
- **并排的元素必须同色**（首页五幕卡片走同一色的明度阶梯）；
  **纵向排列的才可以各走一个色停**（逐日页每一幕、车页每台车）。
- 状态色收敛到两种：`--acc`（已确认）+ `--alert` 陶土红（有问题）；「待办」只用中性描边。
- 甘特图四条轨改成**同一色相的三级明度阶梯 + 一个中性**，不再是四个色相打架。
- 环境光三团用同一个 `--glow`，滚动时由 `anim.js` 改这一个变量、CSS 过渡 2.4s 化过去。
